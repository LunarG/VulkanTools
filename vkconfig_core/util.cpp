...
std::vector<std::string> SplitArgs(const std::string& value) {
    ...
    // Step 1: Tokenize by space, keeping characters inside quotes intact
    for (char ch : value) {
        if (ch == '"') {
            in_quotes = !in_quotes;
            current += ch;  // Keep the quote to identify boundaries later
    ...
    // Step 2: Group tokens into logical arguments
    std::vector<std::string> split_result;

    auto IsOption = [](const std::string& tok) {
        if (tok.empty()) return false;
        if (tok[0] == '-') return true;
        // Recognize quoted options like "--argC=value C" or "--argF" as options
        if (tok[0] == '"' && tok.size() > 1 && tok[1] == '-') return true;
        return false;
    };

    auto IsClosedQuote = [](const std::string& tok) { return tok.size() >= 2 && tok.front() == '"' && tok.back() == '"'; };

    for (const auto& token : tokens) {
        if (split_result.empty()) {
            split_result.push_back(token);
        } else if (!IsOption(token) && IsOption(split_result.back()) && !IsClosedQuote(split_result.back())) {
            // Only swallow the value if the option wasn't explicitly isolated in quotes
            split_result.back() += " " + token;
        } else {
            split_result.push_back(token);
        }
    }

    // Step 3: Strip outer quotes used to delimit an entire argument
    for (auto& arg : split_result) {
