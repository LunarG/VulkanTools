...
The sample Intel driver included in this repository is not supported on Windows, it is excluded from the build.

## Git the Bits

The public repository for VulkanTools is hosted at https://github.com/LunarG/.

The main development branch is "masterXXX", where XXX is an incrementing branch version number. Choose the latest (highest XXX version number) to clone and work off from.
Each masterXXX branch is based off of a given version of the "trunk" branch which
is a mirror of LoaderAndValidationLayers repository (see https://github.com/KhronosGroup/Vulkan-LoaderAndValidationLayers).
...
on GitHub to integrate that work back into the repo.  The pull request should be based on the latest masterXXX branch.

## Linux System Requirements
The sample Intel driver uses DRI3 for its window system interface.
...
```
sudo apt-get install git subversion cmake libgl1-mesa-dev freeglut3-dev libglm-dev qt5-default libpciaccess-dev libpthread-stubs0-dev libudev-dev bison graphviz libpng-dev libxcb1-dev
...
git clone -b <masterXXX> -o LunarG git@github.com:LunarG/VulkanTools.git <YOUR_DEV_DIRECTORY>
#masterXXX is the specific master branch you want to clone (eg "master001").
...
The build generates the loader, layers, demos, vktrace, and tests.
...
./update_external_sources.sh  # Fetches and builds LunarGLASS, glslang and spirv-tools
...
## Validation and sample driver Test
