 ...
 */"""
    ...
    def _get_printf_params(self, vk_type, name, output_param):
        ...
        if "VkMemoryRequirements" in vk_type:
            return ("%p {size=%llu, alignment=%llu, memoryTypeBits=%0x08X}", "%s, (%s == NULL)?0:%s->size, (%s == NULL)?0:%s->alignment, (%s == NULL)?0:%s->memoryTypeBits" % (name, name, name, name, name, name, name), "")
    ...
    def _generate_init_funcs(self):
        ...
        init_tracer.append('            trim::trim_initialize();')
        init_tracer.append('        }')
        init_tracer.append('    }')
...
#                                                      'finalize_txt': 'vktrace_finalize_buffer_address(pHeader, (void**)&(pPacket->pCreateInfos))'},
    ...
    def _generate_trim_statetracking_instructions(self, proto):
        ...
        if 'GetDeviceQueue' is proto.name:
            trim_instructions.append("        trim::Trim_ObjectInfo* pInfo = trim::trim_add_Queue_object(*pQueue);")
            ...
            trim_instructions.append("        trim::Trim_ObjectInfo* pInfo = trim::trim_add_CommandPool_object(*pCommandPool);")
            trim_instructions.append("        pInfo->belongsToDevice = device;")
            ...
            trim_instructions.append("            trim::trim_add_Allocator(pAllocator);")
            trim_instructions.append("        }")
        elif 'FreeCommandBuffers' is proto.name:
            trim_instructions.append("        trim::Trim_ObjectInfo* pCBInfo = trim::trim_get_CommandBuffer_objectInfo(pCommandBuffers[0]);")
            ...
            trim_instructions.append("        trim::Trim_ObjectInfo* pInfo = trim::trim_get_CommandPool_objectInfo(commandPool);")
            ...
            trim_instructions.append("        {")
            trim_instructions.append("            trim::trim_remove_CommandBuffer_object(pCommandBuffers[i]);")
            trim_instructions.append("            trim::trim_remove_CommandBuffer_calls(pCommandBuffers[i]);")
            trim_instructions.append("        }")
        elif 'QueueBindSparse' is proto.name:
            trim_instructions.append("        if (result == VK_SUCCESS) {")
            trim_instructions.append("            trim::Trim_ObjectInfo* pFenceInfo = trim::trim_get_Fence_objectInfo(fence);")
            ...
            trim_instructions.append("                    trim::Trim_ObjectInfo* pInfo = trim::trim_get_Semaphore_objectInfo(pBindInfo->pWaitSemaphores[i]);")
            ...
            trim_instructions.append("                    trim::Trim_ObjectInfo* pInfo = trim::trim_get_Semaphore_objectInfo(pBindInfo->pSignalSemaphores[i]);")
            trim_instructions.append("                    if (pInfo != NULL) {")
            ...
            trim_instructions.append("        trim::Trim_ObjectInfo* pInfo = trim::trim_add_Semaphore_object(*pSemaphore);")
            trim_instructions.append("        pInfo->belongsToDevice = device;")
            ...
            trim_instructions.append("        trim::trim_remove_Semaphore_object(semaphore);")
        elif 'CreateFence' is proto.name:
            trim_instructions.append("        trim::Trim_ObjectInfo* pInfo = trim::trim_add_Fence_object(*pFence);")
            ...
            trim_instructions.append("            trim::Trim_ObjectInfo* pFenceInfo = trim::trim_get_Fence_objectInfo(pFences[i]);")
            trim_instructions.append("            if (pFenceInfo != NULL && result == VK_SUCCESS) {")
            trim_instructions.append("                // clear the fence")
            trim_instructions.append("                pFenceInfo->ObjectInfo.Fence.pendingOnQueue = VK_NULL_HANDLE;")
            trim_instructions.append("            }")
            trim_instructions.append("        }")
            trim_instructions.append("        if (g_trimIsPreTrim) { vktrace_delete_trace_packet(&pHeader); }")
        elif 'GetFenceStatus' is proto.name:
            trim_instructions.append("        trim::Trim_ObjectInfo* pFenceInfo = trim::trim_get_Fence_objectInfo(fence);")
            trim_instructions.append("        if (pFenceInfo != NULL && result == VK_SUCCESS) {")
            trim_instructions.append("            // clear the fence")
            trim_instructions.append("            pFenceInfo->ObjectInfo.Fence.pendingOnQueue = VK_NULL_HANDLE;")
            trim_instructions.append("        }")
            trim_instructions.append("        if (g_trimIsPreTrim) { vktrace_delete_trace_packet(&pHeader); }")
        elif 'WaitForFences' is proto.name:
            trim_instructions.append("        for (uint32_t i = 0; i < fenceCount; i++) {")
            trim_instructions.append("            trim::Trim_ObjectInfo* pFenceInfo = trim::trim_get_Fence_objectInfo(pFences[i]);")
            trim_instructions.append("            if (pFenceInfo != NULL && result == VK_SUCCESS) {")
            trim_instructions.append("                if (waitAll) {")
            trim_instructions.append("                    // clear the fence")
            trim_instructions.append("                    pFenceInfo->ObjectInfo.Fence.pendingOnQueue = VK_NULL_HANDLE;")
            trim_instructions.append("                } else {")
            trim_instructions.append("                    // need to check which specific fence(s) are now clear")
            trim_instructions.append("                    VkResult tmpResult = mdd(device)->devTable.GetFenceStatus(device, pFences[i]);")
            trim_instructions.append("                    if (tmpResult == VK_SUCCESS) {")
            trim_instructions.append("                        pFenceInfo->ObjectInfo.Fence.pendingOnQueue = VK_NULL_HANDLE;")
            trim_instructions.append("                    }")
            trim_instructions.append("                }")
            trim_instructions.append("            }")
            ...
            trim_instructions.append("        trim::trim_remove_Fence_object(fence);")
            ...
            trim_instructions.append("        trim::Trim_ObjectInfo* pInfo = trim::trim_get_Image_objectInfo(dstImage);")
            trim_instructions.append("        if (pInfo != NULL) {")
            trim_instructions.append("            pInfo->ObjectInfo.Image.mostRecentLayout = dstImageLayout;")
            trim_instructions.append("        }")
            trim_instructions.append("        if (g_trimIsPreTrim) { trim::trim_add_CommandBuffer_call(commandBuffer, pHeader); }")
        elif ('CmdClearColorImage' is proto.name or
              'CmdClearDepthStencilImage' is proto.name):
            trim_instructions.append("        trim::Trim_ObjectInfo* pInfo = trim::trim_get_Image_objectInfo(image);")
            trim_instructions.append("        if (pInfo != NULL) {")
            ...
            trim_instructions.append("        if (g_trimIsPreTrim) { trim::trim_remove_CommandBuffer_calls(commandBuffer); }")
        elif 'CreateImageView' is proto.name:
            trim_instructions.append("        trim::Trim_ObjectInfo* pInfo = trim::trim_add_ImageView_object(*pView);")
            trim_instructions.append("        pInfo->belongsToDevice = device;")
            ...
            trim_instructions.append("        trim::trim_remove_ImageView_object(imageView);")
        elif 'CreateImage' is proto.name:
            trim_instructions.append("#if TRIM_USE_ORDERED_IMAGE_CREATION")
            trim_instructions.append("        if (g_trimIsPreTrim) { trim::trim_add_Image_call(pHeader); }")
            trim_instructions.append("#endif //TRIM_USE_ORDERED_IMAGE_CREATION")
            trim_instructions.append("        trim::Trim_ObjectInfo* pInfo = trim::trim_add_Image_object(*pImage);")
            trim_instructions.append("        pInfo->belongsToDevice = device;")
            trim_instructions.append("#if !TRIM_USE_ORDERED_IMAGE_CREATION")
            ...
            trim_instructions.append("        pInfo->ObjectInfo.Image.aspectMask = trim::trim_getImageAspectFromFormat(pCreateInfo->format);")
            ...
            trim_instructions.append("        if (g_trimIsPreTrim) { trim::trim_add_Image_call(pHeader); }")
            trim_instructions.append("#endif //TRIM_USE_ORDERED_IMAGE_CREATION")
            trim_instructions.append("        trim::trim_remove_Image_object(image);")
            ...
            trim_instructions.append("        trim::Trim_ObjectInfo* pInfo = trim::trim_add_BufferView_object(*pView);")
            trim_instructions.append("        if (pInfo != NULL) {")
            trim_instructions.append("            pInfo->belongsToDevice = device;")
            ...
            trim_instructions.append("        trim::trim_remove_BufferView_object(bufferView);")
        elif 'CreateBuffer' is proto.name:
            trim_instructions.append("        trim::Trim_ObjectInfo* pInfo = trim::trim_add_Buffer_object(*pBuffer);")
            trim_instructions.append("        if (pInfo != NULL) {")
            trim_instructions.append("            pInfo->belongsToDevice = device;")
            ...
            trim_instructions.append("        trim::trim_remove_Buffer_object(buffer);")
        elif ('BindBufferMemory' is proto.name):
            trim_instructions.append("        trim::Trim_ObjectInfo* pInfo = trim::trim_get_Buffer_objectInfo(buffer);")
            trim_instructions.append("        if (pInfo != NULL) {")
            ...
            trim_instructions.append("            pInfo->ObjectInfo.Buffer.needsStagingBuffer = trim::trim_IsMemoryDeviceOnly(device, memory);")
            trim_instructions.append("        }")
        elif 'CreateSampler' is proto.name:
            trim_instructions.append("        trim::Trim_ObjectInfo* pInfo = trim::trim_add_Sampler_object(*pSampler);")
            trim_instructions.append("        pInfo->belongsToDevice = device;")
            ...
            trim_instructions.append("        trim::trim_remove_Sampler_object(sampler);")
        elif 'CreateDescriptorSetLayout' is proto.name:
            trim_instructions.append("        trim::Trim_ObjectInfo* pInfo = trim::trim_add_DescriptorSetLayout_object(*pSetLayout);")
            trim_instructions.append("        pInfo->belongsToDevice = device;")
            ...
            trim_instructions.append("        trim::Trim_ObjectInfo* pInfo = trim::trim_get_DescriptorSetLayout_objectInfo(descriptorSetLayout);")
            trim_instructions.append("        if (pInfo != NULL) { delete[] pInfo->ObjectInfo.DescriptorSetLayout.pBindings; }")
            trim_instructions.append("        trim::trim_remove_DescriptorSetLayout_object(descriptorSetLayout);")
        elif 'ResetDescriptorPool' is proto.name:
            trim_instructions.append("        trim::Trim_ObjectInfo* pPoolInfo = trim::trim_get_DescriptorPool_objectInfo(descriptorPool);")
            trim_instructions.append("        pPoolInfo->ObjectInfo.DescriptorPool.numSets = 0;")
            trim_instructions.append("        trim::trim_reset_DescriptorPool(descriptorPool);")
        elif 'CreatePipelineLayout' is proto.name:
            trim_instructions.append("        trim::Trim_ObjectInfo* pInfo = trim::trim_add_PipelineLayout_object(*pPipelineLayout);")
            trim_instructions.append("        pInfo->belongsToDevice = device;")
            ...
            trim_instructions.append("        trim::trim_remove_PipelineLayout_object(pipelineLayout);")
        elif 'DestroyRenderPass' is proto.name:
            trim_instructions.append("        trim::trim_remove_RenderPass_object(renderPass);")
        elif 'CreateShaderModule' is proto.name:
            trim_instructions.append("        trim::Trim_ObjectInfo* pInfo = trim::trim_add_ShaderModule_object(*pShaderModule);")
            trim_instructions.append("        pInfo->belongsToDevice = device;")
            ...
            trim_instructions.append("        if (g_trimIsInTrim) { trim::trim_remove_ShaderModule_object(shaderModule); }")
        elif 'DestroyPipeline' is proto.name:
            trim_instructions.append("        trim::trim_remove_Pipeline_object(pipeline);")
        elif 'DestroyDescriptorPool' is proto.name:
            trim_instructions.append("        trim::trim_remove_DescriptorPool_object(descriptorPool);")
        elif 'DestroyFramebuffer' is proto.name:
            trim_instructions.append("        trim::trim_remove_Framebuffer_object(framebuffer);")
        elif 'CreateEvent' is proto.name:
            trim_instructions.append("        trim::Trim_ObjectInfo* pInfo = trim::trim_add_Event_object(*pEvent);")
            trim_instructions.append("        if (pInfo != NULL) {" )
            trim_instructions.append("            pInfo->belongsToDevice = device;")
            ...
            trim_instructions.append("        trim::trim_remove_Event_object(event);")
        elif 'CreateQueryPool' is proto.name:
            trim_instructions.append("        trim::Trim_ObjectInfo* pInfo = trim::trim_add_QueryPool_object(*pQueryPool);")
            trim_instructions.append("        if (pInfo != NULL) {" )
            trim_instructions.append("            pInfo->belongsToDevice = device;")
            ...
            trim_instructions.append("        trim::trim_remove_QueryPool_object(queryPool);")
        elif 'AcquireNextImageKHR' is proto.name:
            trim_instructions.append("        if (result == VK_SUCCESS && semaphore != VK_NULL_HANDLE) {")
            trim_instructions.append("            trim::Trim_ObjectInfo* pInfo = trim::trim_get_Semaphore_objectInfo(semaphore);")
            trim_instructions.append("            if (pInfo != NULL) {")
            ...
            trim_instructions.append("        {")
            trim_instructions.append("            trim::Trim_ObjectInfo* pInfo = trim::trim_get_PhysicalDevice_objectInfo(physicalDevice);")
            trim_instructions.append("            if (pInfo != NULL)")
            trim_instructions.append("            {")
            ...
            if (ext.name.lower() == extensionName.lower()) or ((extensionName.lower() == 'vk_core') and (ext.name.lower() in approved_ext)):
                for proto in ext.protos:
                    if proto.name in manually_written_hooked_funcs:
                        ...
                        func_body.append('        {')
                        func_body.append('            trim::trim_add_recorded_packet(pHeader);')
                        func_body.append('        }')
                        func_body.append('    }')
