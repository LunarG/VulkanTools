...
#                                                      'finalize_txt': 'vktrace_finalize_buffer_address(pHeader, (void**)&(pPacket->pCreateInfos))'},
    ...
    def _generate_trim_statetracking_instructions(self, proto):
        ...
        if 'GetDeviceQueue' is proto.name:
            ...
            trim_instructions.append("            Trim_ObjectInfo* pFenceInfo = trim_get_Fence_objectInfo(fence);")
            ...
            trim_instructions.append("                    Trim_ObjectInfo* pInfo = trim_get_Semaphore_objectInfo(pBindInfo->pWaitSemaphores[i]);")
            ...
            trim_instructions.append("                    Trim_ObjectInfo* pInfo = trim_get_Semaphore_objectInfo(pBindInfo->pSignalSemaphores[i]);")
            trim_instructions.append("                    if (pInfo != NULL) {")
            ...
            trim_instructions.append("        if (g_trimIsPreTrim) { trim_remove_CommandBuffer_calls(commandBuffer); }")
            ...
            trim_instructions.append("        if (g_trimIsPreTrim) { trim_add_Image_call(pHeader); }")
            ...
            trim_instructions.append("            Trim_ObjectInfo* pInfo = trim_get_Semaphore_objectInfo(semaphore);")
            trim_instructions.append("            if (pInfo != NULL) {")
