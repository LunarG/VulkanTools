...
//=============================================================================
vktraceviewer_QTimelineItemDelegate::vktraceviewer_QTimelineItemDelegate(QObject *parent) : QAbstractItemDelegate(parent) {
    assert(parent != NULL);
}

vktraceviewer_QTimelineItemDelegate::~vktraceviewer_QTimelineItemDelegate() {}

void vktraceviewer_QTimelineItemDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option,
...
//-----------------------------------------------------------------------------
vktraceviewer_QTimelineView::~vktraceviewer_QTimelineView() { m_threadIdList.clear(); }
...
bool vktraceviewer_QTimelineView::event(QEvent *e) {
    if (e->type() == QEvent::ToolTip) {
        ...
        if (index.isValid()) {
            vktrace_trace_packet_header *pHeader = (vktrace_trace_packet_header *)index.internalPointer();
            QToolTip::showText(pHelp->globalPos(),
...
QRectF vktraceviewer_QTimelineView::viewportRect(const QModelIndex &index) const {
    ...
    if (rectf.isValid()) {
        ...
        rectf.adjust(-horizontalScrollBar()->value(), 0, -horizontalScrollBar()->value(), 0);

        // the margin is added last so that it is NOT scaled
        rectf.adjust(m_margin, 0, m_margin, 0);
    }
...
QModelIndex vktraceviewer_QTimelineView::indexAt(const QPoint &point) const {
    if (model() == NULL) return QModelIndex();
...
//-----------------------------------------------------------------------------
QList<uint32_t> vktraceviewer_QTimelineView::getModelThreadList() const { return m_threadIdList; }
...
void vktraceviewer_QTimelineView::drawTimelineItem(QPainter *painter, const QModelIndex &index) {
    ...
    if (!rect.isValid() || rect.bottom() < 0 || rect.y() > viewport()->height() || rect.x() > viewport()->width() ||
        rect.right() < 0) {
        return;
    }
    ...
    if (selectionModel()->isSelected(index)) option.state |= QStyle::State_Selected;
    if (currentIndex() == index) option.state |= QStyle::State_HasFocus;
