...
AppSingleton::AppSingleton(QString single_application_name, int timeout) {
    // If we can connect to the server, it means there is another copy running
    QLocalSocket localSocket;
    localSocket.connectToServer(single_application_name);
    ...
    _is_first = true;
    _local_server.listen(single_application_name);
}

AppSingleton::~AppSingleton() { _local_server.close(); }
