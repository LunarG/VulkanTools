...
void MainWindow::profileItemChanged(QTreeWidgetItem *item, int column) {
    ...
    if (column == 1) {  // Profile name
        ...
        const QString full_path = GetFullPath(DIRECTORY_CONFIGURATION, configuration_item->configuration._name);
...
void MainWindow::RemoveClicked(ConfigurationListItem *item) {
    ...
    // Delete the file
    const QString &full_path = GetFullPath(DIRECTORY_CONFIGURATION, item->configuration._name);
    remove(full_path.toUtf8().constData());
...
void MainWindow::ImportClicked(ConfigurationListItem *item) {
    ...
    QString full_suggested_path = PathManager::Get().GetDirectory(DIRECTORY_IMPORT_CONFIGURATION);
...
void MainWindow::ExportClicked(ConfigurationListItem *item) {
    ...
    QString full_suggested_path = GetFullPath(DIRECTORY_EXPORT_CONFIGURATION, item->configuration._name);
