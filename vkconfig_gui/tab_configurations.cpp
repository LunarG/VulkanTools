...
bool TabConfigurations::EventFilter(QObject *target, QEvent *event) {
    ...
    if (configurator.GetExecutableScope() == EXECUTABLE_NONE || !ui->configurations_list->isEnabled()) {
        ...
        if (right_click) {
            ...
            QAction *action_export_config = new QAction("Export the Layers Configuration file...", nullptr);
            action_export_config->setEnabled(item != nullptr);
            menu.addAction(action_export_config);

            QAction *action_export_settings = new QAction("Export vk_layer_settings.txt layers Settings file...", nullptr);
...
void TabConfigurations::OnContextMenuExportExtensionCodeClicked(ListItem *item) {
    ...
    const QString &selected_path =
        QFileDialog::getSaveFileName(&this->window, "Export VK_EXT_layer_settings layer settings C++ code",
                                     path_export.AbsolutePath().c_str(), "VK_EXT_layer_settings C++ code (*.h)");
