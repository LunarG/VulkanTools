...
TabDrivers::TabDrivers(MainWindow &window, std::shared_ptr<Ui::MainWindow> ui) : Tab(TAB_DRIVERS, window, ui) {
    ...
    if (configurator.vulkan_system_info.loaderVersion < Version(1, 4, 316)) {
        this->ui->tab_widget->setTabEnabled(TAB_DRIVERS, false);
        this->ui->tab_widget->setTabToolTip(TAB_DRIVERS, "Additional Vulkan Drivers require Vulkan Loader 1.4.315 or newer");
    }
...
void TabDrivers::UpdateUI_LayersPaths(UpdateUIMode ui_update_mode) {
    (void)ui_update_mode;
...
void TabDrivers::on_layers_validate_checkBox_toggled(bool checked) {
    Configurator &configurator = Configurator::Get();
    configurator.layers.validate_manifests = checked;
}

void TabDrivers::on_layers_append_pressed() { this->LoadLayersManifest(this->ui->layers_path_lineedit->text()); }

void TabDrivers::on_layers_browse_pressed() {
    Configurator &configurator = Configurator::Get();
...
void TabDrivers::on_layers_reload_pressed() {
...
void TabDrivers::LoadLayersManifest(const QString &selected_path) {
    Configurator &configurator = Configurator::Get();
